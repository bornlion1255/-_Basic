...
def get_query_params():
    """
    Обёртка над API query-параметров, совместимая с разными версиями Streamlit.
    На новых версиях используем st.query_params, на старых — experimental_get_query_params.
    """
    try:
        # Streamlit 1.30+ — QueryParams (dict-подобный),
        # приводим к формату {key: [value, ...]} как у experimental_get_query_params
        qp = st.query_params
        return {
...
def clear_query_params():
    """
    Очищает query‑параметры в адресной строке (если это поддерживается).
    """
    try:
        # Новый API
